...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    let pass = Math.floor(Math.random() * characters.length); // Визначаємо випадковий індекс символу зі списку characters та округляємо до найбільшого цілого, яке менше.
    let randomCharacters = characters.charAt(pass); // Отримуємо символ з випадковим індексом.
    password += randomCharacters; // Додаємо отриманий випадковий символ до паролю.
  }
  return password;
}
...
function calculateCircleArea(radius) {
  ...
  if (typeof radius !== "number") {
    log("Errors"); // Якщо радіус не є числом, виводимо в консоль повідомлення про помилку.
    return null; // Повертаємо null, щоб показати, що обчислення не можливе.
  } else {
    let areaCircle = Math.PI * Math.pow(radius, 2); // Обчислення площі кола за формулою PI * r^2, де PI - число Пі, а r - радіус.
    return areaCircle; // Повертаємо обчислену площу кола.
  }
}
...
function findMinMax(numbers) {
  ...
  if (!Array.isArray(numbers)) {
    ...
    return { min, max }; // Повертаємо об'єкт {min,max}, що містить знайдені мінімальне та максимальне число.
  }
}
...
function calculateHypotenuse(a, b) {
  ...
  if (typeof a !== "number" || typeof b !== "number") {
    log("Error"); // Якщо довжини катетів не є числами, виводимо в консоль повідомлення про помилку.
    return null; // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  } else {
    let lengthOfhypotenuse = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2)); // Обчислюємо довжину гіпотенузи за теоремою Піфагора. c=√(a² + b²)
    return { lengthOfhypotenuse }; // Повертаємо обчислену довжину гіпотенузи.
  }
}
...
function roundObjectValues(obj) {
  ...
  if (typeof obj !== "object") {
    ...
    if (obj === null) {
      log("Помилка: аргумент має бути об'єктом"); // Якщо аргумент не є об'єктом або є null, виводимо повідомлення "Помилка: аргумент має бути об'єктом"
      return null; // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
    } else {
      let arr = Object.entries(obj); // Отримуємо масив пар [ключ, значення] з об'єкта.
      // Перебереємо за допомогою методу map кожну пару [ключ, значення].
      arr = arr.map(([key, value]) => {
        // Перевіряємо, чи значення є числом.
        if (typeof value === "number") {
          value = Math.round(value); // Якщо значення є числом, округлюємо його до найближчого цілого.
          //   value = key
          return [key, value]; // та повертаємо нову пару [ключ, значення]
        } else {
          return [key, value]; // Якщо значення не є числом, повертаємо оригінальну пару [ключ, значення].
        }
      });
      //   log(arr);
      let newObj = Object.fromEntries(arr); // Конвертуємо масив пар [ключ, значення] назад в об'єкт за допомогою Object.fromEntries().
      return newObj; // Повертаємо новий об'єкт з заокругленими значеннями числових властивостей.
    }
  }
}
...
function calculateVolumeCylinder(radius, height) {
  ...
  if (typeof radius !== "number" && typeof height !== "number") {
    ...
    let v = Math.ceil(Math.PI * Math.pow(radius, 2) * height); // Обчислюємо об'єм циліндра за формулою V = PI * r^2 * h, де PI - число Пі, r - радіус, h - висота. Округляємо об'єму до найменшого цілого числа числа що більше.
    return v; // Повертаємо обчислений об'єм.
  }
}
...
function getFractionalPart(num) {
  ...
  if (typeof num !== "number") {
    ...
    let x = Math.trunc(num); // Округляємо відкидуючи дробову частину.
    // log(x);
    let y = num - x; // Віднімаємо цілу частину від початкового числа, щоб отримати дробову частину.
    // log(y);
    y = parseFloat(y); // Округлюємо дробову частину з формату double до float.
    return y; // Повертаємо дробову частину числа.
  }
}
...
function compareAndRound(num1, num2) {
  ...
  if (typeof num1 !== "number" || typeof num2 !== "number") {
    ...
    let max = Math.max(num1, num2); // Визначаємо найбільше число.
    // log(max);
    max = Math.round(max); // Округляємо найбільше число до найближчого цілого.
    return max; // Повертаємо округлене число.
  }
}
...
function estimateInvestment(principal, interestRate, years) {
  ...
  if (typeof (principal || interestRate || years) !== "number") {
    log("Error");
    return null; // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  } else {
    let sum = principal * Math.pow(1 + interestRate, years); // Розраховуємо суму інвестицій за формулою P*(1+rate)^years, де P - початкова сума інвестицій, rate - річна процентна ставка, years - кількість років.
    // log(sum);
    return Math.round(sum); // Округляємо ії до найближчого цілого та Повертаємо розраховану суму інвестицій.
  }
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (!Array.isArray(products)) {
    log("Error"); // Якщо products не є масивом, виводимо повідомлення про помилку.
    return null; // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  } else {
    // Перевіряємо, чи аргумент maxPrice є числом.
    if (typeof maxPrice !== "number") {
      log("Error"); // Якщо maxPrice не є числом, виводимо повідомлення про помилку.
      return null; // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
    } else {
      // Використовуємо метод reduce() для обчислення загальної ціни.
      let totalPrice = products.reduce((accumulator, currentValue) => {
        let sum = accumulator + currentValue.price; // Додаємо ціну продукту до аккумулятора.
        // log(sum);
        return sum;
      }, 0);
      totalPrice = Math.fround(totalPrice); // Конвертуємо totalPrice та maxPrice за допомогою Math.fround.
      maxPrice = Math.fround(maxPrice);
      return totalPrice > maxPrice; // Порівнюємо, чи не перевищує totalPrice maxPrice.
    }
  }
}
